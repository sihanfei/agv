...
rslidarDriver::rslidarDriver(ros::NodeHandle &node)
{
  ...
  node.param("frame_id", config_.frame_id, std::string("odom"));
  ...
  node.param("model", config_.model, std::string("RS16"));
  ...
  node.param("npackets", config_.npackets, npackets);
  ROS_INFO_STREAM("publishing " << config_.npackets << " packets per scan");

  int msop_udp_port;
  node.param("msop_port", msop_udp_port, ( int )MSOP_DATA_PORT_NUMBER);
  int difop_udp_port;
  node.param("difop_port", difop_udp_port, ( int )DIFOP_DATA_PORT_NUMBER);
  ...
  node.param("output_packets_topic", output_packets_topic, std::string("rslidar_packets"));
  msop_output_ = node.advertise< rslidar_msgs::rslidarScan >(output_packets_topic, 10);

  std::string output_difop_topic;
  node.param("output_difop_topic", output_difop_topic, std::string("rslidar_packets_difop"));
  ...
  node.param("time_synchronization", time_synchronization_, false);
  ...
  else // standard behaviour
  {
    ...
    if (time_synchronization_)
    {
      ...
      */
      ...
      sync_header.header.stamp = ros::Time::now();
...
void rslidarDriver::callback(uint32_t level)
{
