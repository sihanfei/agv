...
static const size_t packet_size = sizeof(velodyne_msgs::VelodynePacket().data); // packet_size = 1206 每个packet字节数
 ...
 */
Input::Input(ros::NodeHandle &node, uint16_t port) : private_nh_(node), port_(port)
{
  private_nh_.param("device_ip", devip_str_, std::string(""));
  std::cout << "device_ip : " << devip_str_ << std::endl;
  if (!devip_str_.empty())
...
InputSocket::InputSocket(ros::NodeHandle &private_nh, uint16_t port) : Input(private_nh, port)
{
  ...
  ROS_INFO_STREAM("Opening UDP socket: port " << port); // port = 2368
  sockfd_ = socket(PF_INET, SOCK_DGRAM, 0);             // UDP socket
  if (sockfd_ == -1)
  {
  ...
  // my_addr.sin_addr.s_addr = inet_addr(devip_str_.c_str());
