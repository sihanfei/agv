...
namespace velodyne_rawdata
{
  ...
  double RawData::unpack_vlp16(const velodyne_msgs::VelodynePacket &pkt, DataContainerBase& data)
  {
    ...
    for (int block = 0; block < BLOCKS_PER_PACKET; block++) {			// BLOCKS_PER_PACKET = 12

      // ignore packets with mangled or otherwise different contents
      if (UPPER_BANK != raw->blocks[block].header) {				// UPPER_BANK = oxeeff(big endian)
	...
	if(raw_azimuth_diff < 0)//raw->blocks[block+1].rotation - raw->blocks[block].rotation < 0)
      ...
      for (int firing=0, k=0; firing < VLP16_FIRINGS_PER_BLOCK; firing++){               // VLP16_FIRINGS_PER_BLOCK = 2
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){          // VLP16_SCANS_PER_FIRING = 16    RAW_SCAN_SIZE = 3
          velodyne_pointcloud::LaserCorrection &corrections = 
