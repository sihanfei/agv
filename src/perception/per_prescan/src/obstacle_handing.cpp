...
LocationMathInfo cur_lidar_location_info;
...
int findAgvLocationFroDecision(double x_, double y_, double heading_, int last_pos_index, int isFastJudge,
                               int isHeadingJudge)
{
  double last_distance = -1.0;
  double cur_distance = -1.0;
  ...
  if (!decision_info.path_data_REF.empty())
  {
    int count = static_cast<int>(decision_info.path_data_REF.size());
    ...
    for (int i = begin_index; i < count; i++)
    {
      if (i > 2)
      {
        last_distance = mathPointDistanceSquare(x_, y_, decision_info.path_data_REF.at(i - 2).x,
                                                decision_info.path_data_REF.at(i - 2).y);
        cur_distance = mathPointDistanceSquare(x_, y_, decision_info.path_data_REF.at(i - 1).x,
                                               decision_info.path_data_REF.at(i - 1).y);
        next_distance =
            mathPointDistanceSquare(x_, y_, decision_info.path_data_REF.at(i).x, decision_info.path_data_REF.at(i).y);
        delta_heading = abs(decision_info.path_data_REF.at(i - 1).theta - heading_);
        min_delta_heading = min(min_delta_heading, delta_heading);
        // ROS_INFO("Find ref point at %d (%lf,%lf) ref heading:%lf agv:%lf", i - 1,
        //          rount_ref_line.REF_line_INFO.at(i - 1).rx, rount_ref_line.REF_line_INFO.at(i - 1).ry,
        //          rount_ref_line.REF_line_INFO.at(i - 1).rtheta, heading_);
...
void recvFusionLocationCallback(const location_msgs::FusionDataInfo::ConstPtr &location_msg)
{
  ...
  // cur_location.ref_index = location_index;

  cur_location_info.time = location_msg->header.stamp.toSec();
  cur_location_info.pos_x = location_msg->pose.x;
  cur_location_info.pos_y = location_msg->pose.y;
  cur_location_info.pos_z = location_msg->pose.x;
  cur_location_info.yaw = location_msg->yaw;
  cur_location_info.pitch = location_msg->pitch;
  cur_location_info.roll = location_msg->roll;

  if (location_index % 8 == 1)
  {
    int i = updateLineLocationCoefficient(last_location_info, cur_location_info, cur_location_coefficient);

    // ROS_ERROR("Loc Info Cur T %lf (%lf,%lf) %lf --- Last T %lf (%lf,%lf) %lf --- coe (%lf,%lf) %lf",
    //           cur_location_info.time, cur_location_info.pos_x, cur_location_info.pos_y, cur_location_info.yaw,
    //           last_location_info.time, last_location_info.pos_x, last_location_info.pos_y, last_location_info.yaw,
    //           cur_location_coefficient.pos_x, cur_location_coefficient.pos_y, cur_location_coefficient.yaw);
  ...
  cur_location.decision_index = findAgvLocationFroDecision(cur_location.pos_x, cur_location.pos_y, cur_location.heading,
                                                           cur_location.decision_index, 0, 0);
}
...
double subRadian(double radin_)
{
  if (radin_ > 360.0)
  {
...
int mathPointsShift(const LocationMathInfo &location_e, const LocationMathInfo &location_c,
                    LocationMathInfo &location_o)
{
  if (location_c.updata_tip == 1)
  {
    double time_delta = location_o.time - location_e.time;
    location_o.pos_x = location_e.pos_x + location_c.pos_x * time_delta;
    location_o.pos_y = location_e.pos_y + location_c.pos_y * time_delta;
    location_o.pos_z = location_e.pos_z + location_c.pos_z * time_delta;
    double radian_temp = location_e.yaw + location_c.yaw * time_delta;
    location_o.yaw = subRadian(radian_temp);
    radian_temp = location_e.pitch + location_c.pitch * time_delta;
    location_o.pitch = subRadian(radian_temp);
    radian_temp = location_e.roll + location_c.roll * time_delta;
    location_o.roll = subRadian(radian_temp);
    // location_o.yaw    = subRadian(location_o.yaw);
    // location_o.pitch  = subRadian(location_o.pitch);
    // location_o.roll   = subRadian(location_o.roll);

    // location_o.updata_tip = 1;
    return 1;
  }
  else
  {
...
void recvLidarObsCallback(const perception_sensor_msgs::ObjectList::ConstPtr &msg)
{
  ...
  if (obs_num > 0)
  {
    ...
    lidar_location_obs_time.pos_x = msg->pose.x;
    lidar_location_obs_time.pos_y = msg->pose.y;
    lidar_location_obs_time.heading = msg->yaw;

    cur_lidar_location_info.time = msg->header.stamp.toSec();

    cur_lidar_location_info.updata_tip =
        mathPointsShift(cur_location_info, cur_location_coefficient, cur_lidar_location_info);
    if (cur_lidar_location_info.updata_tip != 1)
    {
      cur_lidar_location_info = cur_location_info;

      // ROS_WARN("use cur loc lidar obs %d %lf %lf %lf TIP = %d", obs_num, cur_lidar_location_info.pos_x,
      //          cur_lidar_location_info.pos_y, cur_lidar_location_info.yaw, cur_lidar_location_info.updata_tip);
    }
    else
    {
      // ROS_INFO("recv lidar obs %d T %lf (%lf,%lf) %lf TIP %d cur T %lf (%lf,%lf) %lf", obs_num,
      //          cur_lidar_location_info.time, cur_lidar_location_info.pos_x, cur_lidar_location_info.pos_y,
      //          cur_lidar_location_info.yaw, cur_lidar_location_info.updata_tip, cur_location_info.time,
      //          cur_location_info.pos_x, cur_location_info.pos_y, cur_location_info.yaw);
    }
...
void detectionCameraObsJudge(LocationTemp &location_obs_time,
                             std::vector<common_msgs::DetectionInfo> &detection_obs_vec)
{
  // int ans = getLocationFromVectorTemp(location_obs_time);
  int ans = 1;
  if (ans == 1)
  {
    ...
    // rvizPointPub(location_obs_time.pos_x, location_obs_time.pos_y);
    ROS_INFO("location_obs_time (%lf,%lf) heading %lf", location_obs_time.pos_x, location_obs_time.pos_y,
             location_obs_time.heading);
    ...
    //障碍物判断初值
    int id_index = 0;
    double radian_ = location_obs_time.heading * M_PI / 180;
    double x_ = location_obs_time.pos_x;
    double y_ = location_obs_time.pos_y;
    double temp_x_ = 0;
    double temp_y_ = 0;
    //更新查询参考线位置
    int ref_index = cur_location.ref_index;
    // if (ref_index > 0 && ref_index < (static_cast< int >(rount_ref_line.REF_line_INFO.size()) - 2))
    // if (ref_index > -1 && ref_index < (static_cast< int >(rount_ref_line.REF_line_INFO.size())))
    if (ref_index > -1 && location_vec.size() > 0)
    {
      int obs_num = static_cast<int>(detection_obs_vec.size());
      for (int i = 0; i < obs_num; i++)
      {
        ObstacleDetectionTemp obstacle_temp;

        obstacle_temp.ref_index = ref_index;
        obstacle_temp.obs_id = detection_obs_vec.at(i).id;

        obstacle_temp.max_x = max(max(detection_obs_vec.at(i).peek.at(0).x, detection_obs_vec.at(i).peek.at(1).x),
                                  max(detection_obs_vec.at(i).peek.at(2).x, detection_obs_vec.at(i).peek.at(3).x));
        obstacle_temp.min_x = min(min(detection_obs_vec.at(i).peek.at(0).x, detection_obs_vec.at(i).peek.at(1).x),
                                  min(detection_obs_vec.at(i).peek.at(2).x, detection_obs_vec.at(i).peek.at(3).x));
        obstacle_temp.max_y = max(max(detection_obs_vec.at(i).peek.at(0).y, detection_obs_vec.at(i).peek.at(1).y),
                                  max(detection_obs_vec.at(i).peek.at(2).y, detection_obs_vec.at(i).peek.at(3).y));
        obstacle_temp.min_y = min(min(detection_obs_vec.at(i).peek.at(0).y, detection_obs_vec.at(i).peek.at(1).y),
                                  min(detection_obs_vec.at(i).peek.at(2).y, detection_obs_vec.at(i).peek.at(3).y));

        // obstacle_temp.max_x     = 0 - detection_obs_vec.at(i).state[0];
        // obstacle_temp.min_y     = detection_obs_vec.at(i).state[1];
        // obstacle_temp.min_x     = 0 - detection_obs_vec.at(i).state[2];
        // obstacle_temp.max_y     = detection_obs_vec.at(i).state[3];
        ...
        if (abs(obstacle_temp.min_y) > 0.0)
        {
          ...
          if (judge_tip > 0)
          {
            ...
            lidarRvizPusInit(points, id_index, 0, 0, 1);
    ...
    {
      ROS_INFO("Error ref_index %d", ref_index);
    }
  }
  else
  {
    ROS_INFO("Can not find location_temp");
  }
}
...
void detectionLidarObsJudge()
{
  ...
  {
    obstacle_detection_vec.clear();
    obstacle_detection_right_vec.clear();
    obstacle_detection_left_vec.clear();
    obstacle_detection_front_vec.clear();
  }
  ...
  double radian_ = cur_lidar_location_info.yaw * M_PI / 180;
  double x_ = cur_lidar_location_info.pos_x;
  double y_ = cur_lidar_location_info.pos_y;
  double temp_x_ = 0;
  double temp_y_ = 0;
  //更新查询参考线位置
  ...
  for (lidar_it = lidar_detection_obs_vec.begin(); lidar_it != lidar_detection_obs_vec.end(); lidar_it++)
  {
    ...
    if (abs(obstacle_temp.min_y) > 0.0)
    {
      ...
      temp_x_ = 0 - lidar_it->peek[0].x;
      temp_y_ = lidar_it->peek[0].y;
      doAgvTransformationToLocal(temp_x_, temp_y_, x_, y_, radian_, obstacle_temp.corner_point[0]);

      temp_x_ = 0 - lidar_it->peek[1].x;
      temp_y_ = lidar_it->peek[1].y;
      doAgvTransformationToLocal(temp_x_, temp_y_, x_, y_, radian_, obstacle_temp.corner_point[1]);

      temp_x_ = 0 - lidar_it->peek[2].x;
      temp_y_ = lidar_it->peek[2].y;
      doAgvTransformationToLocal(temp_x_, temp_y_, x_, y_, radian_, obstacle_temp.corner_point[2]);

      temp_x_ = 0 - lidar_it->peek[3].x;
      temp_y_ = lidar_it->peek[3].y;
      doAgvTransformationToLocal(temp_x_, temp_y_, x_, y_, radian_, obstacle_temp.corner_point[3]);
      ...
      if (obstacle_temp.min_y > 9 && obstacle_temp.min_y < 40 &&
          min(abs(obstacle_temp.max_x), abs(obstacle_temp.min_x)) < 1.9)
      {
        judge_tip = 1;
        obstacle_detection_vec.push_back(obstacle_temp);
      }

      if (obstacle_temp.min_y > -19 && obstacle_temp.min_y < 40 && obstacle_temp.max_x >= -6.0 &&
          obstacle_temp.max_x <= -2.0)
      {
        judge_tip = 1;
...
void agvContourLineRvizInitClear(visualization_msgs::Marker &points, int id_index = 0)
{
  points.header.frame_id = "/odom";
  points.header.stamp = ros::Time::now();
  points.ns = "/map/agv_contour_line";
  // points.action             = visualization_msgs::Marker::DELETE;
  points.action = visualization_msgs::Marker::DELETEALL;
  points.pose.orientation.w = 1.0;
  points.id = id_index;
...
void agvContourLineRvizInit(visualization_msgs::Marker &points)
{
  points.header.frame_id = "/odom";
  points.header.stamp = ros::Time::now();
  points.ns = "/map/agv_contour_line";
...
int pushAGVContourLineUseRef(int ref_index_cur, int ref_index_b = 20, int ref_index_f = 30)
{
  if (ref_index_cur < 0)
  {
    return -1;
  }
      int ref_decision_count = static_cast<int>(rount_ref_line.REF_line_INFO.size());
  int index_b_ = ref_index_cur;
  int index_f_ = ref_index_cur;

  double agv_w = 3.0;
  double agv_l = 16.0;

  if ((ref_index_cur - ref_index_b) <= 0)
  {
    index_b_ = 0;
  }
  else
  {
    index_b_ = ref_index_cur - ref_index_b;
  }

  if ((ref_index_cur + ref_index_f) >= ref_decision_count)
  {
    index_f_ = ref_decision_count;
  }
  else
  {
    index_f_ = ref_index_cur + ref_index_f;
  }

  visualization_msgs::MarkerArray maker_temp;
  visualization_msgs::Marker contour_points;
  agvContourLineRvizInitClear(contour_points);
  maker_temp.markers.push_back(contour_points);
  contour_pub.publish(maker_temp);

  maker_temp.markers.clear();

  agvContourLineRvizInit(contour_points);

  for (size_t i = index_b_; i < index_f_; ++i)
  {
    double radian_ = rount_ref_line.REF_line_INFO.at(i).rtheta * M_PI / 180;
    double x_ = rount_ref_line.REF_line_INFO.at(i).rx;
    double y_ = rount_ref_line.REF_line_INFO.at(i).ry;

    geometry_msgs::Point p;
    p.x = x_ + agv_l / 2 * sin(radian_) - agv_w / 2 * cos(radian_);
    p.y = y_ + agv_l / 2 * cos(radian_) + agv_w / 2 * sin(radian_);
    p.z = 0;
    contour_points.points.push_back(p);
    p.x = x_ + agv_l / 2 * sin(radian_) + agv_w / 2 * cos(radian_);
    p.y = y_ + agv_l / 2 * cos(radian_) - agv_w / 2 * sin(radian_);
    p.z = 0;
    contour_points.points.push_back(p);
...
int pushAGVContourLineUseDec(int ref_index_cur, int ref_index_b = 20, int ref_index_f = 30)
{
  if (ref_index_cur < 0)
  {
    return -1;
  ...
  int ref_decision_count = static_cast<int>(decision_info.path_data_REF.size());
  int index_b_ = ref_index_cur;
  int index_f_ = ref_index_cur;
  ...
  for (size_t i = index_b_; i < index_f_; ++i)
  {
    double radian_ = decision_info.path_data_REF.at(i).theta * M_PI / 180;
    double x_ = decision_info.path_data_REF.at(i).x;
    double y_ = decision_info.path_data_REF.at(i).y;
...
int main(int argc, char *argv[])
{
  ...
  rviz_left_boundary_point_pub_ = n.advertise<geometry_msgs::PointStamped>("/obs/veh_left_point_boundary_msg", 5, true);
  contour_pub = n.advertise<visualization_msgs::MarkerArray>("/map/agv_contour_msg", 5, true);
  ...
  while (ros::ok())
  {
    ...
    if (cur_location.ref_index > 0)
    {
      ...
      if (generate_tip == 0)
      {
        pushAGVContourLineUseRef(cur_location.ref_index);
      }
      else
      {
        pushAGVContourLineUseDec(cur_location.decision_index);
      }
