...
pair<float, float> sensor_lidar::calculateMeanVxVy(vector<float>& sx_list, vector<float>& sy_list, vector<float>& detal_t_list)
{
...
float sensor_lidar::calculateVariance(vector<float>& resultSet)
{
    float variance = 0.0;
    if(resultSet.size() >= 2)
    {
