...
// 计算IOU
float sensor_lidar::calculateIOU(Eigen::VectorXf new_object_state, Eigen::VectorXf global_object_state)
{
...
// 速度相似度
float sensor_lidar::calculateVelocitySimilarity(vector<float>& new_vx_list, vector<float>& new_vy_list, vector<float>& old_vx_list, vector<float>& old_vy_list)
{
