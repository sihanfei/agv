...
RTKControl::RTKControl(ros::NodeHandle &nh) : nh_(nh)
{
  ROS_INFO("into");
  ...
  if (mode == 0)
  {
    control_vcu_pub_ = nh.advertise< geometry_msgs::PoseStamped >("/Prescan/Control/Command", BUF_LEN, true);
  }
  else
  {
  ...
  if (mode == 0)
  {
    pos_sub_ = nh.subscribe("/Prescan/Pos/track", 10, &RTKControl::recvPrescanRealtimePosCallback, this);
  }
  else
  {
    pos_sub_ = nh.subscribe("/drivers/can_wr/imu_gnss_msg", 10, &RTKControl::recvHuacePosCallback, this);
  }
...
void RTKControl::getRouteFromFile()
{
  //获取routing data path
  ...
  {
    ROS_INFO("Open route_data.bin error!");
    exit(1);
  }
  else
  {
    ROS_INFO("route_date.bin is open!");
    while (!feof(route_data_read_fp))
    {
      fscanf(route_data_read_fp, "%u %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
             &read_position.n_gps_sequence_num, &read_position.x, &read_position.y, &read_position.z,
             &read_position.lon, &read_position.lat, &read_position.height, &read_position.velocity,
             &read_position.velocity_x, &read_position.velocity_y, &read_position.velocity_z, &read_position.heading,
             &read_position.pitch, &read_position.roll, &read_position.dist);
...
void RTKControl::rtkControl()
{
  ...
  while (ros::ok())
  {
    ...
    if (start_tip_ == 2 && math_tip_ == 2)
    {
      ...
      if (pathtype == 0)
      {
        min_index              = findRealMinIndex_ringpath(min_index_theory, xy_pos_temp);
        min_index_front_center = findRealMinIndex_ringpath(min_index_theory_front_center, xy_pos_temp_front_center);
        findPoints2ComputeRadius_ringpath(min_index, indexr1, indexf1, indexf2, indexf3);
      }
      else
      {
      ...
      if (pathtype == 0)
      {
        findPrePoint_ringpath(pre_pos_temp, xy_pos_temp, min_index, R_tmp);
      }
      else
      {
      ...
      if (mode == 0)
      {
        ...
        prescan_control_command.pose.position.x = control_angle_f;
        prescan_control_command.pose.position.y = control_angle_r;

        if (pathtype == 0)
        {
          prescan_control_command.pose.position.z = computespeed(R_tmp, straight_speed);
        }
        else
        {
          if ((route_section_index_[1]+1 - min_index_theory) * equal_length <= start2stop_dist)
          {
            prescan_control_command.pose.position.z = sqrt((route_data_[route_section_index_[1]].x - route_data_[min_index_theory].x) *
                                                               (route_data_[route_section_index_[1]].x - route_data_[min_index_theory].x) +
                                                           (route_data_[route_section_index_[1]].y - route_data_[min_index_theory].y) *
                                                               (route_data_[route_section_index_[1]].y - route_data_[min_index_theory].y)) *
                                                      (speed_limit / start2stop_dist);
            if((route_section_index_[1]+1 - min_index_theory) <= 2)
            {
              prescan_control_command.pose.position.x = 0;
              prescan_control_command.pose.position.y = 0;
            }
          }
          else
          {
            prescan_control_command.pose.position.z = computespeed(R_tmp, straight_speed);
            if(prescan_control_command.pose.position.z < 0.5)
            {
              prescan_control_command.pose.position.z = 0.5;
            }
          }
        }

        // speed limitation
        if (prescan_control_command.pose.position.z > speed_limit)
        {
          prescan_control_command.pose.position.z = speed_limit;
        }
 
        // velocity orientation
        prescan_control_command.pose.position.z = forward_backward_flag*prescan_control_command.pose.position.z;
      ...
      else
      {
        ...
        else
        {
          if ((route_section_index_[1]+1 - min_index_theory) * equal_length <= start2stop_dist)
          {
            control_msg.Vel_Req = sqrt((route_data_[route_section_index_[1]].x - route_data_[min_index_theory].x) *
                                           (route_data_[route_section_index_[1]].x - route_data_[min_index_theory].x) +
                                       (route_data_[route_section_index_[1]].y - route_data_[min_index_theory].y) *
                                           (route_data_[route_section_index_[1]].y - route_data_[min_index_theory].y)) *
                                  (speed_limit / start2stop_dist);
...
void RTKControl::forward_backward_judge()
{
...
double RTKControl::getR(const positionConf &pt1, const positionConf &pt2, const positionConf &pt3)
{
...
double RTKControl::ComputeSigma(const double &R,const positionConf &_nearst_pos_,const positionConf &_xy_pos_,const positionConf &_pre_pos_,double &_control_angle_f,double &_control_angle_r)
{
  double sig;
...
double RTKControl::computeheading(const positionConf &_p1_,const positionConf &_p2_)
{
  double heading = 0;
  if(_p1_.x == _p2_.x)
  {
...
double RTKControl::getMinR(const double &R1, const double &R4, const double &R5)
{
  double min_tmp;
  if (R1 <= R4)
  {
...
void RTKControl::findPoints2ComputeRadius(const int &index, int &index_r1, int &index_f1, int &index_f2, int &index_f3)
{
...
void RTKControl::findPoints2ComputeRadius_ringpath(const int &index, int &index_r1, int &index_f1, int &index_f2,
                                                   int &index_f3)
{
  double dl = (center2frontaxis + center2rearaxis) / 2;
...
void RTKControl::angle_constrain(double &input_value)
{
...
void RTKControl::yawrate_constrain(const double &timenow, const double &timeprevious, double &control_angle,
...
void RTKControl::gps2xy(positionConf &xy_p, const positionConf &real_p)
{
...
void RTKControl::prescanxy2xy(positionConf &xy_p, const positionConf &real_p)
{
  xy_p.x          = real_p.lon;
  xy_p.y          = real_p.lat;
...
void RTKControl::xy2vxy(positionConf &targetp_veh, const positionConf &targetp_xy, const positionConf &xy_p)
{
...
double RTKControl::computespeed(const double &R, const double &straightline_speed)
{
  return 2 / M_PI * atan(R / 50) * straightline_speed;
}

double RTKControl::computedheading(const double &_heading1_,const double &_heading2_)
{
  double dheading = 0;
...
double RTKControl::StanleyPreviewLatteralModel(const positionConf &xy_p, const positionConf &vnxy_p,
                                               const positionConf &vnxy_p_fc, const positionConf &vpxy_p)
{
  ...
  // ROS_INFO("D_err:%f yaw:%f preyaw:%f stanley:%f",D_err,yaw,preyaw,atan2(-k_stanley*vnxy_p_fc.x,(k_soft +
  // xy_p.velocity))*180/M_PI);
  return D_err;
}

void RTKControl::findDiagPrePoint(positionConf &pre_p, const positionConf &real_p, const int &index, const double &R)
{
  double pre_length;
...
void RTKControl::findPrePoint(positionConf &pre_p, const positionConf &real_p, const int &index, const double &R)
{
  double pre_length;

  pre_length = (2 * (basic_prelength_max - basic_prelength_min)) / (1 + exp(-(R - Rmax))) + basic_prelength_min +
               real_p.velocity * preview_time;
  //ROS_INFO("prelength is: %f   velocity is: %f", pre_length, real_p.velocity);
  double l       = 0;
  int s          = route_section_index_[1]+1;
  int iter_index = index + 1;
  while (l < pre_length)
...
void RTKControl::findPrePoint_ringpath(positionConf &pre_p, const positionConf &real_p, const int &index,
                                       const double &R)
{
  double pre_length;
...
int RTKControl::findRealMinIndex(const int &min_index_theory_, const positionConf &real_p)
{
...
int RTKControl::findRealMinIndex_ringpath(const int &min_index_theory_, const positionConf &real_p)
{
  double delay_length = real_p.velocity * sys_delay;
  double l            = 0;
  int s               = route_data_.size();
...
void RTKControl::insgps2center(positionConf &xy_p)
{
...
void RTKControl::center2frontaxis_tf(positionConf &xy_p)
{
  xy_p.x = xy_p.x + center2frontaxis * sin(xy_p.heading * M_PI / 180);
  xy_p.y = xy_p.y + center2frontaxis * cos(xy_p.heading * M_PI / 180);
}

double RTKControl::pointDistanceSquare(const positionConf &xy_p, const positionConf &route_point)
{
...
double RTKControl::norm2(const double &dx, const double &dy)
{
  return sqrt(dx * dx + dy * dy);
}

void RTKControl::findStrangePoints()
{
...
void RTKControl::route_cut(const positionConf &rpt)
{
  if(previous_route_data_.size() == route_data_.size() && previous_route_data_[0].x == route_data_[0].x && previous_route_data_[0].y == route_data_[0].y && previous_route_data_[previous_route_data_.size()-1].x == route_data_[route_data_.size()-1].x && previous_route_data_[previous_route_data_.size()-1].y == route_data_[route_data_.size()-1].y)//与上一帧路径相同
  {
...
int RTKControl::findClosestRefPoint(const positionConf &rpt,const int &previous_loc_num)
{
  float min_value = 1000000; // 1km范围内
  double ans      = 100000;
  int loc_num     = route_data_.size() + 1;
...
int RTKControl::findClosestFCRefPoint(const positionConf &rpt,const int &ref_loc_num)
{
  float min_value = 1000000; // 1km范围内
  double ans      = 100000;
...
void RTKControl::recvVehDataCallback(const control_msgs::com2veh::ConstPtr &msg)
{
  real_position_.velocity = msg->VelSpeed / 3.6;
}

void RTKControl::recvbestposCallback(const novatel_gps_msgs::NovatelPositionConstPtr &msg)
{
...
void RTKControl::recvInspvaCallback(const novatel_gps_msgs::InspvaConstPtr &msg)
{
  ...
  // real_position_.velocity = sqrt(msg->north_velocity*msg->north_velocity + msg->east_velocity*msg->east_velocity +
  // msg->up_velocity*msg->up_velocity);
...
void RTKControl::recvHuacePosCallback(const location_sensor_msgs::IMUAndGNSSInfo &msg)
{
...
void RTKControl::recvFusionPosCallback(const location_msgs::FusionDataInfo &msg)
{
}

void RTKControl::recvPrescanRealtimePosCallback(const geometry_msgs::PoseStamped &msg)
{
