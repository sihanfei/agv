...
DIAGControl::DIAGControl(ros::NodeHandle &nh) : nh_(nh)
{
  ROS_INFO("into");
  ...
  //收
  // bestpos_sub_ = nh.subscribe("bestpos",10, &DIAGControl::recvbestposCallback,this);
  // Inspva_sub_ = nh.subscribe("inspva",10, &DIAGControl::recvInspvaCallback,this);
  // velocity_sub_ = nh.subscribe("com2vehmsg",10,&DIAGControl::recvVehDataCallback,this);
  if (mode == 0)
  {
    pos_sub_ = nh.subscribe("/Prescan/Pos/track", 10, &DIAGControl::recvPrescanRealtimePosCallback, this);
  }
  else
  {
    pos_sub_ = nh.subscribe("/drivers/can_wr/imu_gnss_msg", 10, &DIAGControl::recvHuacePosCallback, this);
  }
...
void DIAGControl::getRouteFromFile()
{
  //获取routing data path
  ...
  else
  {
    ...
    // ROS_INFO("route_data_[1]lon:%.8lf, lat:%.8lf",route_data_[1].lon,route_data_[1].lat);

    math_tip_ = 2;
  }
}

void DIAGControl::dgControl()
{
  ROS_INFO("into-2");
  ros::Rate loop_rate(FRE);
  int ss = route_data_.size();
  ...
  while (ros::ok())
  {
    ...
    if (start_tip_ == 2 && math_tip_ == 2)
    {
      ...
      ROS_INFO("Rtmp is: %f  R1 is: %f  R4 is: %f  R5 is: %f", R_tmp, R1, R4, R5);
      ...
      control_angle_r        = control_angle_f;
      ...
      if (mode == 0)
      {
        ...
        else
        {
          ...
          {
            prescan_control_command.pose.position.z = sqrt((route_data_[ss - 1].x - route_data_[min_index].x) *
      ...
      else
      {
        ...
        else
        {
          if ((ss - min_index) * equal_length <= start2stop_dist)
          {
            control_msg.Vel_Req = sqrt((route_data_[ss - 1].x - route_data_[min_index].x) *
                                           (route_data_[ss - 1].x - route_data_[min_index].x) +
                                       (route_data_[ss - 1].y - route_data_[min_index].y) *
                                           (route_data_[ss - 1].y - route_data_[min_index].y)) *
                                  (speed_limit / start2stop_dist);
...
double DIAGControl::getR(const positionConf &pt1, const positionConf &pt2, const positionConf &pt3)
{
...
double DIAGControl::ComputeSigma(const double &R)
{
  double sig;
...
double DIAGControl::getMinR(const double &R1, const double &R4, const double &R5)
{
  double min_tmp;
  if (R1 <= R4)
  {
...
void DIAGControl::findPoints2ComputeRadius(const int &index, int &index_r1, int &index_f1, int &index_f2, int &index_f3)
{
...
void DIAGControl::findPoints2ComputeRadius_ringpath(const int &index, int &index_r1, int &index_f1, int &index_f2,
                                                   int &index_f3)
{
  double dl = (center2frontaxis + center2rearaxis) / 2;
  int s     = route_data_.size();
...
void DIAGControl::angle_constrain(double &input_value)
{
...
void DIAGControl::yawrate_constrain(const double &timenow, const double &timeprevious, double &control_angle,
...
void DIAGControl::gps2xy(positionConf &xy_p, const positionConf &real_p)
{
...
void DIAGControl::prescanxy2xy(positionConf &xy_p, const positionConf &real_p)
{
  xy_p.x          = real_p.lon;
  xy_p.y          = real_p.lat;
...
void DIAGControl::xy2vxy(positionConf &targetp_veh, const positionConf &targetp_xy, const positionConf &xy_p)
{
...
double DIAGControl::computespeed(const double &R, const double &straightline_speed)
{
  return 2 / M_PI * atan(R / 50) * straightline_speed;
}

double DIAGControl::StanleyPreviewLatteralModel(const positionConf &xy_p, const positionConf &vnxy_p,
...
void DIAGControl::findPrePoint(positionConf &pre_p, const positionConf &real_p, const int &index, const double &R)
{
  double pre_length;
  /*
...
void DIAGControl::findPrePoint_ringpath(positionConf &pre_p, const positionConf &real_p, const int &index,
                                       const double &R)
{
  double pre_length;
  /*
...
int DIAGControl::findRealMinIndex(const int &min_index_theory_, const positionConf &real_p)
{
...
int DIAGControl::findRealMinIndex_ringpath(const int &min_index_theory_, const positionConf &real_p)
{
  double delay_length = real_p.velocity * sys_delay;
  double l            = 0;
  int s               = route_data_.size();
...
void DIAGControl::insgps2center(positionConf &xy_p)
{
...
void DIAGControl::center2frontaxis_tf(positionConf &xy_p)
{
  xy_p.x = xy_p.x + center2frontaxis * sin(xy_p.heading * M_PI / 180);
  xy_p.y = xy_p.y + center2frontaxis * cos(xy_p.heading * M_PI / 180);
}

double DIAGControl::pointDistanceSquare(const positionConf &xy_p, const positionConf &route_point)
{
...
double DIAGControl::norm2(const double &dx, const double &dy)
{
  return sqrt(dx * dx + dy * dy);
}

int DIAGControl::findClosestRefPoint(const positionConf &rpt)
{
...
void DIAGControl::recvVehDataCallback(const control_msgs::com2veh::ConstPtr &msg)
{
  real_position_.velocity = msg->VelSpeed / 3.6;
}

void DIAGControl::recvbestposCallback(const novatel_gps_msgs::NovatelPositionConstPtr &msg)
{
  ...
  // ROS_INFO("receive %d %u %.8lf
  // %.8lf",num_,msg->novatel_msg_header.sequence_num,real_position_.lon,real_position_.lat);
  start_tip_ = 2;
}

void DIAGControl::recvInspvaCallback(const novatel_gps_msgs::InspvaConstPtr &msg)
{
...
void DIAGControl::recvHuacePosCallback(const location_sensor_msgs::IMUAndGNSSInfo &msg)
{
...
void DIAGControl::recvFusionPosCallback(const location_msgs::FusionDataInfo &msg)
{
}

void DIAGControl::recvPrescanPositionCallback(const visualization_msgs::MarkerConstPtr &msg)
//  void DIAGControl::recvPrescanPositionCallback(const nav_msgs::OdometryConstPtr& msg)
{
}

void DIAGControl::recvPrescanRealtimePosCallback(const geometry_msgs::PoseStamped &msg)
{
...
void DIAGControl::recvRouteCallback(const visualization_msgs::MarkerConstPtr &msg)
//  void DIAGControl::recvPrescanPositionCallback(const nav_msgs::OdometryConstPtr& msg)
{
