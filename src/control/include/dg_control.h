#ifndef DG_CONTROL_H_
#define DG_CONTROL_H_
...
{
public:
  DIAGControl(ros::NodeHandle &nh);
  ~DIAGControl();

  ////回调函数
  //仿真回调函数
  void recvPrescanPositionCallback(const visualization_msgs::MarkerConstPtr &msg);
  void recvRouteCallback(const visualization_msgs::MarkerConstPtr &msg);

  void recvPrescanRealtimePosCallback(const geometry_msgs::PoseStamped &msg);
  //哈弗H7 诺瓦太组合导航定位信息
  ...
  //查找最近点函数
  int findClosestRefPoint(const positionConf &rpt);
  //查找考虑延迟最近点函数
  ...
  //计算前后轮转向比函数
  double ComputeSigma(const double &R);
  //查找计算曲率半径的点函数
