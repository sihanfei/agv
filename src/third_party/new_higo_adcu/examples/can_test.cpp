...
void *readLoop(void *pdata)
{
	...
	while(1)
	{
		...
		if(length > 0)
		{
			...
			if(canBuffer.ide == 1)
			{
					printf("id = %08X,dlc = %d,ide = %d,rtr = %d,prio = %d,data=[ ",
...
int main(int argc,char **argv)
{
	...
	if(argc !=4)
	{
		printf("please input correct parameters<channelNumber><BaudRate><IDE>\n");
		return 0;
	}
	channel = atoi(argv[1]);
	...
	{
		printf("please input correct BaudRate<1 ~ 7> : 100k 125k 250k 400k 500k 800k 1000k\n");
		return 0;
	}
	if(atoi(argv[3])<0 || atoi(argv[3])>1)
	{
		printf("please input correct<0 ~ 1>  IDE=0:Standard,IDE=1:Extended\n");
		return 0;
	}

	bzero(data,100);
	adcuCanData canbuf;
	canbuf.ide = atoi(argv[3]);
	canbuf.dlc = 0x08;
	canbuf.rtr = 0x00;
	canbuf.prio=0x00;
	canbuf.id = 0x123;
	...
	int status=0;
	opt_buf[0] = atoi(argv[1])-1;
        opt_buf[1] = atoi(argv[2]);
	...
	while(1)
	{
		...
		{
				printf("ADCU_DEV_STATUS_ABNORMAL\n");
			break;
		}
		for(int i = 0; i<3; i++)
		{
