...
void RefSender::readRefFromGetRefLine()
{
  ...
  std::string ref_line_appendix_file_name = home_path + workplace_path + "/ref_line_appendix.json";
...
void RefSender::routeDecisionInfoPub(const vector< RefLaneData > &rrps)
{
  ...
  double begin_speed    = 0.1;
  double end_speed      = 0.1;
  double negative_acc   = 3.0;
  double positive_acc   = 1.0;
  ...
  {
    common_msgs::PathPoint path_point_temp;
    RefLaneData ref_lane_data_temp = rrps.at(i);
    RefLaneData forward_ref_lane_data_temp;

    path_point_temp.x = ref_lane_data_temp.point_x;
    path_point_temp.y = ref_lane_data_temp.point_y;

    double angle_temp     = ref_lane_data_temp.theta;
    path_point_temp.theta = angle_temp * 180 / M_PI; // 2PI -> 360;
    ...
    if (curve_tip == 0)
    {
      ...
      {
        k_                         = k_ + 10;
        forward_ref_lane_data_temp = rrps.at(k_);
    ...
    else
    {
      ...
      path_point_temp.v = 2 - curve_num * 0.1;
    }
    ...
    if (lane_tip_ == 1)
    {
      ...
      if (i > 0)
      {
        begin_speed = path_point_temp.v;
      }
      lane_num++;
      path_point_temp.v = begin_speed + lane_num * 0.1;
      if (path_point_temp.v > 2)
      {
        path_point_temp.v = 2;
      }
    }
    ...
    if (i < 16)
    {
      path_point_temp.v = 0.5 + i * 0.1;
      if (ref_lane_data_temp.kappa == 0 && ref_lane_data_temp.dappa == 0)
      {
      }
      else
      {
        if (path_point_temp.v > 1)
        {
          path_point_temp.v = 1;
        }
      }
    ...
    {
      double distance_ = 0;
      if ((k_ + 10) < max_num_ && end_tip_ == 0)
      {
        forward_ref_lane_data_temp = rrps.at(k_);
        distance_ = pointDistanceSquare(forward_ref_lane_data_temp.point_x, forward_ref_lane_data_temp.point_y,
                                        task_click_.x, task_click_.y);
        if (distance_ < (0.5 * 0.5))
        {
          end_tip_  = 1;
          end_speed = path_point_temp.v;
        }
      }
      ...
      {
        end_num_++;
        path_point_temp.v = path_point_temp.v - end_num_ * end_speed / 10;
        if (path_point_temp.v < 0)
        {
          path_point_temp.v = 0;
        }
      }

      distance_ = pointDistanceSquare(path_point_temp.x, path_point_temp.y, task_click_.x, task_click_.y);
      //结束点判断
      if (distance_ < (0.5 * 0.5))
      {
        end_index_        = i;
    ...
    if (path_point_temp.v > 1.0)
    {
      path_point_temp.v = 1.0;
    }

    ROS_WARN("Point %d (%lf,%lf) MaxSpeed:%lf", i, path_point_temp.x, path_point_temp.y, path_point_temp.v);

    decision_info.path_data_REF.push_back(path_point_temp);
  }
...
void RefSender::routeRefPlanningPub(const vector< RefLaneData > &rrps)
{
  ...
  for (size_t i = 0; i < rrps.size(); i++)
  {
    ...
    double angle_temp = ref_lane_data_temp.theta;
    // double angle_temp = 2 * M_PI - ref_lane_data_temp.theta;
    // if (angle_temp < 0)
    // {
    //   angle_temp = angle_temp + 2 * M_PI;
    // }
    ref_point_.rtheta = angle_temp * 180 / M_PI; // 2PI -> 360
#else
    ref_point_.rtheta = angleChange(ref_lane_data_temp.theta);
#endif

    ROS_WARN("Pub angle %lf -> %lf", ref_lane_data_temp.theta, ref_point_.rtheta);

    ref_point_.rkappa    = ref_lane_data_temp.kappa; // 1/m
    ref_point_.rdkappa   = ref_lane_data_temp.dappa; // 1/m2
    ...
    // ref_point_.width_left = 2;  // m
    // ref_point_.width_right = 2; // m
    // if (i % 100 == 0)
    // {
    //   ROS_INFO("Pub angle %lf -> %lf", ref_lane_data_temp.theta, ref_point_.rtheta);
    // }
...
int RefSender::findLaneIDWithXY(TaskPoint &find_point_in_, int find_mode)
{
  ...
  if (find_vcu_laneID_tip == 1)
  {
    ...
    // find_point_in_.heading = 2 * M_PI - ref_point_temp.theta;
    // if (find_point_in_.heading < 0)
    // {
    //   find_point_in_.heading = find_point_in_.heading + 2 * M_PI;
    //   if (find_point_in_.heading > 2 * M_PI)
    //   {
    //     find_point_in_.heading = 0;
    //   }
    // }
  }
  else
  ...
  if (find_vcu_laneID_tip == 1)
  {
    ...
    {
      ROS_WARN("Find (%lf,%lf) in line %d heading ref %lf  <-> vcu %lf", find_point_in_.x, find_point_in_.y,
               find_point_in_.laneID, ref_point_temp.theta * 180 / M_PI, find_point_in_.heading);
    }
  }
  else
  {
    ROS_INFO("Find (%lf,%lf) in line %d heading %lf", find_point_in_.x, find_point_in_.y, find_point_in_.laneID,
             find_point_in_.heading);
  }
...
int RefSender::getPointFromRoute(const RouteData &rd_, const TaskPoint &vlp_, const vector< RefLaneData > &trrd_,
                                 vector< RefLaneData > &trrd_t_, int &last_, int &now_, int &next_,
                                 const int &last_ret_)
{
...
{
  ROS_INFO("Finding vcu  (%lf,%lf) in map", vcu_location_.x, vcu_location_.y);
  vcu_location_recv_tip = 0;
