...
public:
  ...
  void getBoundrayPointsFromFile2Ver(std::__cxx11::string &file_name, std::vector< BoundrayPoint > &boundray_points_vec)
  {
    ifstream infile;
    ...
    std::stringstream ss;
    int get_boundray_count = 0;
    int add_boundray_count = 0;
    while (getline(infile, str))
    {
      BoundrayPoint boundray_temp;
      ss.clear();
      ss << str.c_str();
      ss2BoundrayPoint(ss, boundray_temp);
      //查找重复
      boundray_points_vec.push_back(boundray_temp);
      ++add_boundray_count;
      ++get_boundray_count;
    }
    infile.close();
    cout << "get total " << get_boundray_count << " line from file, and add " << add_boundray_count
         << " in boundray_points_vec." << endl;
  }

  void ss2BoundrayPoint(std::stringstream &line_ss, BoundrayPoint &out_boundray_line)
  {
    char ss_c_temp;
    line_ss >> out_boundray_line.boundray_id;
    line_ss >> ss_c_temp;
    line_ss >> out_boundray_line.point.x;
    line_ss >> ss_c_temp;
    line_ss >> out_boundray_line.point.y;
    line_ss >> ss_c_temp;
    out_boundray_line.point.z = 0;
  }

  void getBoundrayLineFromFile2Map(std::__cxx11::string &file_name, std::map< int, BoundrayLine > &boundray_line_map)
  {
    ifstream infile;
    infile.open(file_name); //将文件流对象与文件连接起来
    if (!infile)
    ...
    while (getline(infile, str))
    {
      BoundrayLine boundray_temp;
      ss.clear();
      ss << str.c_str();
      ss2BoundrayLine(ss, boundray_temp);
      //查找重复
      std::map< int, BoundrayLine >::iterator it = boundray_line_map.find(boundray_temp.id);
      if (it == boundray_line_map.end())
      {
        boundray_line_map.insert(std::pair< int, BoundrayLine >(boundray_temp.id, boundray_temp));
        ++add_boundray_count;
        cout << "Add " << get_boundray_count << " line " << boundray_temp.id
             << " in boundray_line_map, now map size is " << boundray_line_map.size() << endl;
      }
      else
      {
        cout << "This " << get_boundray_count << " line " << boundray_temp.id
             << " is during in boundray_line_map, now map size is " << boundray_line_map.size() << endl;
    ...
    infile.close();
    cout << "get total " << get_boundray_count << " line from file, and add " << add_boundray_count
         << " in boundray_line_map." << endl;
  }

  void ss2BoundrayLine(std::stringstream &line_ss, BoundrayLine &out_boundray_line)
  {
    char ss_c_temp;
    line_ss >> out_boundray_line.id;
    line_ss >> ss_c_temp;
    line_ss >> out_boundray_line.begin.x;
    line_ss >> ss_c_temp;
    line_ss >> out_boundray_line.begin.y;
    line_ss >> ss_c_temp;
    line_ss >> out_boundray_line.end.x;
    line_ss >> ss_c_temp;
    line_ss >> out_boundray_line.end.y;
    line_ss >> ss_c_temp;
    // cout << "BoundrayLine:"
    //      << " id:" << out_boundray_line.id << " begin: (" << out_boundray_line.begin.x << ","
    //      << out_boundray_line.begin.y << ") end: (" << out_boundray_line.end.x << " ," << out_boundray_line.end.y <<
    //      ")"
    //      << endl;
  }

  void getBoundrayCircleFromFile2Map(std::__cxx11::string &file_name,
                                     std::map< int, BoundrayCircle > &boundray_circle_map)
  {
    ...
    while (getline(infile, str))
    {
      BoundrayCircle boundray_temp;
      ss.clear();
      ss << str.c_str();
      ss2BoundrayCircle(ss, boundray_temp);
      //查找重复
      std::map< int, BoundrayCircle >::iterator it = boundray_circle_map.find(boundray_temp.id);
      if (it == boundray_circle_map.end())
      {
        boundray_circle_map.insert(std::pair< int, BoundrayCircle >(boundray_temp.id, boundray_temp));
        ++add_boundray_count;
        cout << "Add " << get_boundray_count << " line " << boundray_temp.id
             << " in boundray_circle_map, now map size is " << boundray_circle_map.size() << endl;
      }
      else
      {
        cout << "This " << get_boundray_count << " line " << boundray_temp.id
             << " is during in boundray_circle_map, now map size is " << boundray_circle_map.size() << endl;
        // ROS_INFO("The map had same state as %d %s", p_state_->id, AgvStateStr[p_state_->id].c_str());
      }
      ++get_boundray_count;
    }
    infile.close();
    cout << "get total " << get_boundray_count << " line from file, and add " << add_boundray_count
         << " in boundray_circle_map." << endl;
  }

  void ss2BoundrayCircle(std::stringstream &line_ss, BoundrayCircle &out_boundray_circle)
  {
    char ss_c_temp;
    line_ss >> out_boundray_circle.id;
    line_ss >> ss_c_temp;
    line_ss >> out_boundray_circle.centre.x;
    line_ss >> ss_c_temp;
    line_ss >> out_boundray_circle.centre.y;
    line_ss >> ss_c_temp;
    line_ss >> out_boundray_circle.radius;
    line_ss >> ss_c_temp;
    line_ss >> out_boundray_circle.begin_radian;
    line_ss >> ss_c_temp;
    line_ss >> out_boundray_circle.end_radian;
    line_ss >> ss_c_temp;
    // cout << "BoundrayCircle:"
    //      << " id:" << out_boundray_circle.id << " centre: (" << out_boundray_circle.centre.x << ","
    //      << out_boundray_circle.centre.y << ") radius:" << out_boundray_circle.radius
    //      << " begin_radian:" << out_boundray_circle.begin_radian * 180 / M_PI
    //      << " end_radian:" << out_boundray_circle.end_radian * 180 / M_PI << endl;
  }
  ...
  void getRefLineFromFile2Map(std::__cxx11::string &file_name, std::map< int, RefLine > &ref_line_map)
  {
    ...
    while (getline(infile, str))
    {
      ...
      if (it == ref_line_map.end())
      {
        ...
        ++add_refline_count;
        cout << "Add " << get_line_count << " line " << ref_line_temp.ref_line_id
             << " in ref line map, now map size is " << ref_line_map.size() << endl;
    ...
    infile.close();
    cout << "get total " << get_line_count << " line from file, and add " << add_refline_count << " in ref map."
         << endl;
  }
  ...
  void getAppendixFromFile2RefLine(std::__cxx11::string &file_name, std::map< int, RefLine > &ref_line_map)
  {
    ...
    while (getline(infile, str))
    {
      ...
      else
      {
        ...
        cout << "This appendix_id ref line id " << ref_line_id_temp << " is duing RefLineVec, add " << aa_id_num
             << " AA into." << endl;
      }
    }
  }
                               ...
                               std::vector< RefPoints > &ref_points_vec)
  {
    ...
    while (getline(infile, str))
    {
      ...
      if (ref_line_key != ref_point_temp.ref_line_id)
      {
        ...
        if (it == ref_line_map.end())
        {
          cout << "This ref_points ref line id " << ref_point_temp.ref_line_id << " can not find duing RefLineVec"
               << endl;
        ...
        else
        {
          if (ref_line_key != 0)
          {
            cout << "This ref_points ref line id " << ref_line_key << " is duing RefLineVec, add "
                 << ref_point_vec_temp.size() << "point." << endl;
    ...
    infile.close();
    cout << "This ref_points map size is " << ref_points_map.size() << endl;
    cout << "This ref_points vec size is " << ref_points_vec.size() << endl;
  }
  ...
  void ss2AppendixAttribute(std::stringstream &aa_ss, AppendixAttribute &out_appendix_attribute)
  {
    ...
    aa_ss >> ss_c_temp;
    aa_ss >> out_appendix_attribute.ref_line_count;
    aa_ss >> ss_c_temp;
    for (size_t i = 0; i < out_appendix_attribute.ref_line_count; i++)
    {
      int ref_line_id_temp;
      aa_ss >> ref_line_id_temp;
      aa_ss >> ss_c_temp;
      out_appendix_attribute.ref_line_id_list.push_back(ref_line_id_temp);
    }
    ...
    for (size_t i = 0; i < out_appendix_attribute.corner_count; i++)
    {
      ...
      aa_ss >> ss_c_temp;
      corner_point_temp.z = 0.0;
      // aa_ss >> corner_point_temp.z;
      // aa_ss >> ss_c_temp;
      out_appendix_attribute.corner_point.push_back(corner_point_temp);
    }
    // cout << "The AA: " << out_appendix_attribute.appendix_id << " ref_num: " << out_appendix_attribute.ref_line_count
    // << " corner_num: " << out_appendix_attribute.corner_count << endl;
  }
                                     ...
                                     std::map< int, std::vector< RefPoints > > &ref_points_map_caches, int mode)
  {
    ...
    if (!line_distance_map.empty()) //结果不为空
    {
      ...
      cout << "The point is during " << ref_line_id_temp << " ref line distance is " << line_distance_map.begin()->first
           << endl;
    }
    else if (!curve_distance_map.empty())
    {
      ...
      cout << "The point is during " << ref_line_id_temp << " ref curve" << endl;
    }
    else
    {
      ref_line_id_temp = 0;
  ...
  int findXYZFromRefPointVecTraversal(RefPoints &point_in, std::vector< RefPoints > &ref_points_vec_caches)
  {
    ...
    if (!find_points_map_caches.empty()) //结果不为空
    {
      ...
      cout << "The most closest is (" << find_points_map_caches.begin()->second.point.x << " , "
           << find_points_map_caches.begin()->second.point.y << " , " << find_points_map_caches.begin()->second.point.z
           << ") distance is: " << find_points_map_caches.begin()->first << " ref line id is: " << ref_line_id_temp
           << endl;
